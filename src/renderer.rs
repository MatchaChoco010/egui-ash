...
impl<A: Allocator + 'static> ManagedTextures<A> {
    ...
    fn update_texture(&mut self, texture_id: egui::TextureId, delta: egui::epaint::ImageDelta) {
        ...
        let cmd_fence = unsafe { self.device.create_fence(&fence_info, None).unwrap() };
        ...
        unsafe {
            self.device
                .queue_submit(self.queue, &submit_infos, cmd_fence)
                .unwrap();
            self.device
            ...
            if let Some(existing_texture) = existing_texture {
                ...
                unsafe {
                    ...
                    let cmd_buff_begin_info = vk::CommandBufferBeginInfo::builder()
                        ...
                        .build();
                    self.device
                        .begin_command_buffer(cmd, &cmd_buff_begin_info)
                        .unwrap();
